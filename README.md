...
5. Adjust the **Sidebar AI Tool Order** section by dragging tools or using the up/down controls. The order is retained across launches, and **Restore Default** returns to the built-in order.
6. Choose a language in the **Language** section. The default is Simplified Chinese; the selection applies immediately and is retained across launches.
