...
enum PetEventCommand {
    ...
    static func runIfNeeded(arguments: [String] = CommandLine.arguments) -> Bool {
        ...
        guard let provider = values["provider"],
            ...
            FileHandle.standardError.write(Data("Usage: AIUsageBar pet-event --provider <id> --state <working|waiting|blocked|done> [--project <path>] [--message <text>] [--model <name>] [--reasoning-effort <level>] [--subagent] [--subagent-name <name>] [--tokens <n>] [--requests <n>] [--session <id>]\n".utf8))
            return true
        }
        let event = PetHookEvent(
            ...
            isSubagent: values["subagent"].map { ["1", "true", "yes"].contains($0.lowercased()) }
                ?? values["subagent-name"].map { _ in true },
            subagentName: values["subagent-name"],
